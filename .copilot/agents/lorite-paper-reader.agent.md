...
## Mode 2 — Deep-read (one paper → Obsidian literature note, written directly)
Read the **whole** paper, then write the structured summary **directly into `media/research/<title> - <citekey>.md`** as plain markdown. That note is the single source of the reading content. **No Zotero child note**, no `ai_chats/notes/` literature note, and no portable markdown copy. (Zotero remains the bibliographic source of truth for metadata and the PDF, nothing more.)
...
- **Never report an attach as failed on the first read.** `numChildren` and the children endpoint lag a write by a few seconds, so a check made immediately after `zotero_add_by_doi` / `_add_by_url` / `fetch_attach.py` routinely returns `0` for an attachment that did land. **Re-check before saying anything**: `curl "http://localhost:23119/api/users/0/items/<K>/children?format=json"` and look for an `application/pdf` child (normally `linkMode: linked_file`). Only report manual work if the child is still absent on the second look, and say which check you ran. Telling the user to drag a PDF that is already attached is worse than saying nothing.
- **After a fetch, check for an orphaned duplicate PDF.** The add path and `fetch_attach.py` can each leave a copy in `~/nextcloud/zotero/` under different names. If the item's linked child points at one of them, the other is an orphan — say so, and offer to delete it rather than leaving dead megabytes behind.
- **`WebFetch` cannot parse PDFs.** It returns a "corrupted or improperly encoded PDF" message and asks you to paste the text, but it *does* save the raw bytes and prints the saved path. Don't retry it and don't give up: run `pdftotext <saved path> -` on that file, which works. Same for any PDF you download yourself. Reaching for a web search instead of `pdftotext` is the wrong move when you already have the bytes.
