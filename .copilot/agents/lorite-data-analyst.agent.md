...
- **Write-back is numbers/figures/tables only.** On approval you may fill the paper's numeric `%
  ...
  **The paper write-back is your one decision point** — analysis itself (cohort choice per the
  manifest rules, trim mode per the design, re-runs, figure iterations) is safe and repeatable; run
  it without asking, and put anything judgement-laden (an exclusion the user might dispute, a
  confound worth footnoting) in the results summary for reaction rather than as a blocking question.
