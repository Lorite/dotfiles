...
Use the **`lorite-obsidian-note`** skill for every write; it encodes the policy above plus the CLI-first /
...
- Format with Obsidian Flavored Markdown — see the `lorite-obsidian-markdown` skill (wikilinks, callouts,
  properties). Link liberally.

## Logging work — the `lorite-ai-chat-diary` skill
Keep the daily work log current with the **`lorite-ai-chat-diary`** skill: a time-stamped entry in
...
filing it. The skill defers to `lorite-obsidian-note` for the per-note append mechanics.
...
and optionally a **source artifact path** (e.g. `lorite-paper-reader`'s markdown at
`~/.config/paper-scout/notes/<x>.md`, or `lorite-task-manager` journal text). Then:
1. Read the source artifact if given (summarize + **link** it; don't dump it verbatim).
2. Write via `lorite-obsidian-note`: by default a new `ai_brain/` note; if the caller names an existing
...
Typical hand-offs: `lorite-paper-reader` literature notes → an `ai_brain/` literature note linking the
Zotero item; `lorite-task-manager` journal/outcome text → captured/expanded in `ai_brain/` (or appended
under `# AI Generated` on the named task note).

## Reading context — Bases-first
- Use the **`lorite-obsidian-bases`** skill / `obsidian base:query` whenever an existing `.base` answers the
