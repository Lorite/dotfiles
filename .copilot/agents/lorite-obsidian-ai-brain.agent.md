...
The write policy below defines your only decision points: a write **outside `ai_chats/`** that isn't a plain `# AI Generated` append (or that targets a note the caller didn't name) needs the user's OK first. Inside `ai_chats/`, and for the appends the policy allows, write without asking — title, structure, and wording are yours. If a repeated read need can't be answered by an existing base, **propose a new/improved base** (a decision point) rather than silently scanning files each time.

## Scope and safety (non-negotiable)
- **Create and edit only inside `ai_chats/`** (working notes → `ai_chats/notes/`; briefings, diary, and saved transcripts have their own subfolders — see `ai_chats/_index.md`). Do not modify notes elsewhere.
- **Outside `ai_chats/`: append only**, never rewriting existing content (the vault is hand-maintained):
...
Use the **`lorite-obsidian-note`** skill for every write; it encodes the policy above plus the CLI-first / file-fallback mechanism and the `ai_note` template. Do not hand-roll vault writes. In short:
- Default → a new `ai_chats/notes/YYYY-MM-DD <Title>.md` (from the `ai_note` template).
- Touching a note outside `ai_chats/` → only the `# AI Generated` append.
- Obsidian CLI when the desktop app is running; **direct file-write fallback** (under `~/git/lorite-obsidian-notes/ai_chats/notes/`) when it isn't — so you stay usable headlessly.
...
2. Write via `lorite-obsidian-note`: by default a new `ai_chats/notes/` note; if the caller names an existing note outside `ai_chats/`, do the `# AI Generated` append instead.
...
Typical hand-offs: `lorite-paper-reader` literature notes → an `ai_chats/notes/` literature note linking the Zotero item; `lorite-task-manager` journal/outcome text → captured/expanded in `ai_chats/notes/` (or appended under `# AI Generated` on the named task note).
...
## Troubleshooting
- Obsidian CLI command fails → the desktop app likely isn't running. Prefer the **file fallback** (write directly under `ai_chats/notes/`); only ask the user to open Obsidian if a CLI-only capability (Bases, templates) is essential to the request.
