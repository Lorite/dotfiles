...
- **`WebFetch` cannot parse PDFs.** It returns a "corrupted or improperly encoded PDF" message and asks you to paste the text, but it *does* save the raw bytes and prints the saved path. Don't retry it and don't fall back to search snippets: run `pdftotext <saved path> -` on that file, which works. For grounding a definition in a primary source this is the difference between quoting the paper and guessing at it.
- **Search snippets are not grounding.** Secondary sources routinely get parameter values and author lists wrong (observed: a claim that Independence Detection is `B = 1` in MA-CBS when the paper says `B = 0`). For anything load-bearing — a definition, a bound, an author list, a number — read the primary PDF via the route above and cite the section.
- **Verify frontmatter *after* the linter has run, with a YAML parser.** Careful quoting is not the guard: the Obsidian linter rewrites quoted scalars on save (sometimes stripping the quotes entirely) and reflows inline lists to block lists. Re-read the file and parse it after writing. Also confirm `url:` still holds the URL you wrote (`work/concepts`, `personal/concepts` and `media/research` are excluded from `obsidian-local-images-plus` as of 2026-08-04, but re-reading costs nothing and other folders are not).
- **`maturity_level` scale** is defined in `templates/concepts/concept.md`. Use it rather than guessing, and don't treat the template's `1` as mandatory for a note you author from scratch.
