...
- **When verifying YOUR code needs a real FLIGHT, that flight still goes through the experiments framework (2026-08-12)**: hand the coder (or the operator at the rig) a `trial_runner.py --scratch --label <what-it-tests>` invocation against the relevant campaign — full T-0 gate, operator steps, a collision-proof `bags_scratch/` recording that coverage never counts. Never hand-launch a stack plus `ros2 bag record` for a flight: the day this rule was written, that path flew a drone that silently never armed and lost a flight's data to a stale pre-typed bag name. Ground verification (nodes, topics, TF, sim) stays yours and needs no framework.
