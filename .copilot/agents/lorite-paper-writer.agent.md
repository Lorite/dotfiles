...
**ICRA repo file layout** (as of 2026-09-15): `main.tex` is the paper (formerly `main_lorite.tex`, renamed); `ideas_dump.tex` is the idea graveyard where cut paragraphs, diagrams, and references live for reuse and is never compiled into the paper; `outline.tex` is a standalone one-line skeleton of `main.tex` (section headers plus one idea line per paragraph and one line per figure/table) for arguing about structure. Edit prose in `main.tex`; when a section moves, keep `outline.tex` in sync. The old `main_simpler.tex` was removed. Note that a separate repo `~/git/icra2027-markerless-drone-localization` is the Overleaf-synced submission mirror.
...
- **Non-scientific references go in footnotes, not the bibliography.** URLs, vendor and product pages, datasheets, software repositories, and documentation are cited inline as `\footnote{Label, \url{...}}` at first mention (needs `\usepackage{url}` or `hyperref`), not as `\cite{}` entries. The bibliography is reserved for scholarly works: papers, preprints (arXiv items with real authors and a title are scholarly, keep them in `references.bib`), books, and standards. When moving a reference out of the bibliography, attach the footnote to the word or after the sentence's terminal punctuation and leave the `.bib` entry in place (an uncited entry is simply dropped by BibTeX, so no other paper that shares the `.bib` breaks). Do not footnote a scientific citation, and do not `\cite{}` a bare URL.
