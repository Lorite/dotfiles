...
**First, look at what is already running.** `lorite_intent.py status` lists every live declaration and marks any older than 4 hours `<-- STALE`, followed by a `STALE:` summary line. A stale one is almost always a session that forgot to stop, and it is costing the day real accuracy: **a running declaration is written nowhere until it stops**, so it is missing from every CSV and daily note in the meantime. Deal with it before starting yours — stop it if the work is plainly over (`stop --task "<name>"`), or `edit` its end time to the last real activity once it has been written. Say what you did.
...
- **`lorite_intent.py` talks to the LOCAL aw-server** (`AW_SERVER`, default `http://localhost:5600`) and, when `~/.config/lorite/aw-remote.env` exists, **mirrors every block straight to the home server** over Tailscale SSH (it prints `mirrored to ...`). That mirror is not a nicety: declarations are stamped with their start but written at stop, so they arrive out of order, and aw-sync skips events that start before its resume point — one 24-minute block was lost that way on 2026-08-12 and the daily note under-reported the day. `edit` and `rm` delete the mirror's copy too. If the mirror fails it says so on stderr and the local block still stands. If ActivityWatch itself is not running the script exits non-zero with a clear message — say so and carry on, exactly as with a missing STT config.
- **`status` flags stale declarations** (older than 4 h, `LORITE_INTENT_STALE_HOURS` to change), and **`list` prints the real time as well as the plain sum** when a day contains overlap ("4 h 41 m over 12 block(s), 3 h 31 m of real time"). The plain sum is per task and will exceed the day, on purpose.
