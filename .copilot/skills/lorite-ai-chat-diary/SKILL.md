...
- **Inline-maintenance exception (user grant [[2026-07-31]]):** reference notes **wikilinked from the task note driving the current session** may additionally be kept factually current in their hand-written body — add a short sentence/bullet, or strike-and-replace an outdated fact with a date (`~~old~~ new ([[<yyyy-MM-dd>]])`), **never deleting or rewording** the original. Only facts verified by this session's tool output, and **every inline edit is logged in the task note's journal entry**. The AI also **maintains the linking itself**: an AI-owned `## Related notes` subsection in the task note lists the reference notes the task touches (found by keyword search over note names, `aliases:`, and `tags`). And when a verified thing has **no note at all**, the AI may **create** one anywhere in the vault — provided it first proves nothing covers it (names *and* aliases, whole vault), writes only what it verified, and stamps `ai_created: <yyyy-MM-dd>` in the frontmatter so the morning briefing surfaces it for the user's review. The AI may likewise **archive** a note whose subject it verified is no longer used (add the `archived` tag, move to a sibling `archived/` folder, stamp `ai_archived:`), and **tag** a note that has no real tags (path→tags convention + topical tags reused from the existing vocabulary, add-only, stamp `ai_tagged:`, user grant [[2026-08-10]]). Full rules and exclusions in the `lorite-obsidian-note` skill's *Related-notes*, *Inline-maintenance*, *Creation*, *Archiving*, and *Tagging* exceptions.
- Never write secrets.
