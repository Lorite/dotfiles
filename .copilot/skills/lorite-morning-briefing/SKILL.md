...
4. **Write:** only **add** to frontmatter. Insert the `tags:` block (or replace an empty `tags:` key), append to an existing one otherwise, and stamp **`ai_tagged: <yyyy-MM-dd>`** so the note surfaces in this briefing's review greps. **`ai_tagged:` is the single explicit exception to the add-only rule.** If the note already carries one, **replace that line in place, and never append a second**. A stale date hides the note from those greps (25 notes slipped through that way on [[2026-08-11]]), and a **duplicate key breaks Obsidian's property parsing and Bases queries** (one note drifted that way by [[2026-08-18]], carrying both `2026-08-13` and `2026-08-18`). **Verify before moving to the next note: the frontmatter must contain exactly one `ai_tagged:` line.** Apart from that one key, never edit, reorder or remove existing tags, other frontmatter, or the body. Handle all three `tags:` shapes, including the **inline scalar** `tags: knowledge-management media articles` that older wallabag imports use: Obsidian splits it on commas *and* spaces, so append in place (`tags: knowledge-management media articles, work`) and never restructure it into a list, which would merge several real tags into one bogus tag. **Audit the resulting `git diff` for deleted lines**, expecting exactly two kinds and no others: the replaced `ai_tagged:` line, and rewritten inline `tags:` lines. Any other deletion is a bug, so do not trust the write count alone. **Cap at ~20 notes per run** and list any beyond the cap as pending.
...
- **Home-server (headless) run** (`$OBSIDIAN_GUI=0`): the wrapper sets `$VAULT` = Syncthing copy, `$VAULT_GIT` = **the same path** (`~/git/lorite-obsidian-notes`), `$AUDIT_REF=origin/main`. Writes into `$VAULT` sync back to the laptop via Syncthing — the briefing itself does **no git push**. Since **2026-08-12** the vault directory on the server is itself a clone of the GitHub remote (Syncthing excludes `.git/`, so the repo is server-local), and `vault-git-backup.service` commits and pushes it earlier in the same nightly batch, so by the time the briefing runs `origin/main` already holds the night's commit. That replaced the separate `~/git/lorite-obsidian-notes-audit` side clone, a second 2.4 GB copy of the same history, now retired. If the audit reports "commit audit skipped", `$VAULT/.git` is missing — rebuild it by cloning the remote elsewhere, moving its `.git` into `$VAULT`, then `git reset --mixed HEAD` and `git config core.filemode false` (Syncthing sets its own permission bits, which otherwise make every file read as modified).
- **Audit the repo you are standing in, not only its pushed history.** The 2026-08-11 audit missed an entire local-only repo for two weeks because the scripted check only ever read the side clone. Whatever `$VAULT_GIT` points at, also run `git remote -v` and `git log --oneline @{u}..` (plus `du -sh .git`), so a remote-less or unpushed repo cannot quietly accumulate commits again.
