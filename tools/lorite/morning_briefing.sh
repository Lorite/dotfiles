...
# Use the lorite-llm wrapper. NOTE the client is NOT auto-detected for this job: the unit pins
# LLM_CLIENT=claude (and says why). Since 2026-07-31 a pinned client STILL falls back to OpenCode
# when it fails — read the unit's comment before assuming this job is always Claude-audited.
# The MODEL comes from ~/.config/environment.d/lorite-llm.conf (home server: claude-sonnet-5).
