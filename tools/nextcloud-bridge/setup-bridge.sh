...
#
# Creates per-machine symlinks from vault-root names to real Nextcloud sync-client
# folders, so notes can embed/link files with stable vault-relative wikilinks
# (e.g. ![[papers/foo.pdf]]) that resolve identically on every machine — while the
# actual bytes live in Nextcloud (editable by any app, offline-available via the
# sync client). Each link is added to the vault's .stignore AND .gitignore so it is
# NEVER synced: targets differ per host (~/nextcloud on the laptop vs the server's
# data dir), and the /home/<user> path itself differs (lori vs lorite), so every
# machine must create its own link locally.
#
...
# Link vault/<link> -> NEXTCLOUD_BASE/<sub>, then ignore <link> in Syncthing + git.
bridge_one() {
    local sub="$1" link="$2"
    local source="$NEXTCLOUD_BASE/$sub"
    local target="$OBSIDIAN_VAULT/$link"
    ...
    if [ -L "$target" ]; then
        if [ "$(readlink "$target")" = "$source" ]; then
            ok "'$link' → '$source' (already linked)"
        else
            ln -sfn "$source" "$target"
            ok "'$link' re-pointed → '$source'"
        fi
    elif [ -e "$target" ]; then
        ...
        ln -sfn "$source" "$target"
        ok "'$link' → '$source' (linked)"
    fi

    # Anchor with a leading slash so only the vault-root link is ignored.
