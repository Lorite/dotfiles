...
`setup-bridge.sh` symlinks selected **Nextcloud sync-client** folders into a dedicated
subfolder of the Obsidian vault (default `vault/nextcloud/`) so notes can embed/link files
with stable, cross-machine vault-relative wikilinks (`![[nextcloud/papers/foo.pdf]]`) while
the actual bytes live in Nextcloud — editable by any app (e.g. Ubuntu's Documents),
offline-available, and Nextcloud-synced to the home server.
...
1. Creates `vault/<NEXTCLOUD_VAULT_SUBDIR>/<linkname>` → `NEXTCLOUD_BASE/<subfolder>`
   (idempotent; re-points a stale link; **refuses to replace a real, non-symlink vault path**).
2. Adds the whole `/<NEXTCLOUD_VAULT_SUBDIR>` folder to the vault's `.stignore` (Syncthing)
   and `.gitignore` (git) so the links are **never synced** — every machine creates its own,
   because the symlink target and the `/home/<user>` path differ per host (`lori` on the
   laptop, `lorite` on the server). (With `NEXTCLOUD_VAULT_SUBDIR=""`, each root-level link is
   ignored individually instead.)
