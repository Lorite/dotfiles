...
**Signing in to the second instance needs the callback hand-delivered.** The OAuth callback arrives as a `claude://` deep link, and `xdg-open` routes that scheme to exactly one `.desktop` entry (`x-scheme-handler/claude` in `~/.config/mimeapps.list`). Whichever entry is default receives it, so the callback raises the ITU window and the personal instance never gets its token. There is no per-instance routing and no manual code-paste fallback in `app.asar`.
